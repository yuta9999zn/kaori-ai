...
// Auto-converted from 23PlastForm Admin Detail.jsx by convert_jsx_to_tsx.py.
...
const GlobalSidebar = ({  isMobile, activeRoute, setActiveRoute, isCollapsed, setIsCollapsed  }: any) => {
  ...
  const currentHighlight = 
    ...
    (activeRoute === 'admin-invite' || activeRoute === 'admin-details') ? 'admin' : 
    activeRoute;
  ...
  return (
    <PageContainer maxWidth="default" className="relative">
      ...
      <PageHeader 
        ...
        actions={
          <>
            ...
            <Button variant="outline" onClick={() => setIsResetModalOpen(true)} className="hidden sm:flex">
            ...
            <div className="relative group">
              ...
              <div className="absolute right-0 top-full mt-1 w-48 bg-white border border-[var(--border-color)] shadow-soft-md rounded-lg-custom z-50 py-1.5 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-100 focus-within:opacity-100 focus-within:visible">
                ...
                <button className="w-full text-left px-3 py-1.5 text-sm text-[var(--text-primary)] hover:bg-[var(--bg-app)] flex items-center gap-2 transition-colors sm:hidden" onClick={() => setIsResetModalOpen(true)}>
      ...
      <Modal 
        ...
        description={`Send a password reset link to ${adminData.email}.`}
        footer={
          <>
            <Button variant="outline" onClick={() => setIsResetModalOpen(false)} disabled={isProcessing}>Cancel</Button>
            <Button onClick={handleResetConfirm} isLoading={isProcessing}>Send Reset Email</Button>
          </>
        }
      >
        <div className="text-sm text-[var(--text-secondary)]">
          The user will receive an email with a secure link to choose a new password. Their active sessions will not be terminated until the password is changed.
        </div>
      </Modal>
...
export default function KaoriPlatformShell() {
  const [activeRoute, setActiveRoute] = useState('admin-details'); // Set default to Admin Details for demo
