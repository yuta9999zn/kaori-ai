...
// Auto-converted from 7Global Sidebar.jsx by convert_jsx_to_tsx.py.
...
// --- NAVIGATION CONFIGURATION (config/navigation.ts equivalent) ---
...
const GlobalSidebar = ({  isMobile, activeRoute, setActiveRoute, isCollapsed, setIsCollapsed  }: any) => {
  ...
  // Handle click outside for User Menu Dropdown
  useEffect(() => {
  ...
  return (
    ...
    `}>
      ...
      {/* 3. BOTTOM SECTION */}
      <div className="shrink-0 border-t border-[var(--border-color)]/50 p-3 space-y-2">
        ...
        <div className="relative" ref={userMenuRef}>
          <button 
            ...
            className={`
              w-full flex items-center h-12 rounded-md-custom transition-colors border border-transparent
              ${isUserMenuOpen ? 'bg-[var(--bg-app)] border-[var(--border-color)]/50' : 'hover:bg-[var(--bg-app)]'}
              ${collapsed ? 'justify-center px-0' : 'px-2 gap-3'}
            `}
          >
            <div className="w-8 h-8 rounded-md-custom bg-white flex items-center justify-center font-medium text-[var(--primary-gold)] shadow-soft-sm border border-[var(--border-color)] shrink-0">
              A
            </div>
            {!collapsed && (
              <>
                <div className="flex-1 min-w-0 text-left">
                  ...
                  <p className="text-[11px] text-[var(--text-secondary)] truncate">admin@kaori.io</p>
                </div>
          ...
          {isUserMenuOpen && (
             <div 
                ...
                className={`
                absolute bottom-full mb-2 bg-white rounded-lg-custom shadow-soft-md border border-[var(--border-color)] py-1.5 w-[220px] animate-in fade-in zoom-in-95 duration-200 z-50
                ${collapsed ? 'left-full ml-3 bottom-0 mb-0' : 'left-0'}
             `}>
                <div className="px-3 py-2 border-b border-[var(--border-color)]/50 mb-1" role="none">
                   <p className="text-sm font-medium text-[var(--text-primary)]">Admin User</p>
                   <p className="text-xs text-[var(--text-secondary)]">admin@kaori.io</p>
                </div>
                <button role="menuitem" className="w-full text-left px-3 py-2 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-app)] hover:text-[var(--text-primary)] transition-colors flex items-center gap-2">
                  <User className="w-4 h-4" aria-hidden="true" /> Profile Settings
                </button>
                <button 
                  role="menuitem"
                  onClick={() => { setActiveRoute('sessions'); setIsUserMenuOpen(false); }}
                  className="w-full text-left px-3 py-2 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-app)] hover:text-[var(--text-primary)] transition-colors flex items-center gap-2"
                >
                  <Shield className="w-4 h-4" aria-hidden="true" /> Security & Sessions
                </button>
                <div className="h-[1px] bg-[var(--border-color)]/50 my-1" role="none" />
                <button role="menuitem" className="w-full text-left px-3 py-2 text-sm text-[#9B5050] hover:bg-[#FDF8F8] transition-colors flex items-center gap-2 font-medium">
        ...
        {!isMobile && (
          <button 
            ...
            className={`
              w-full flex items-center h-8 rounded-md-custom text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-app)] transition-colors
    ...
    <>
      ...
      <div className="flex h-screen overflow-hidden bg-[var(--bg-app)] text-[var(--text-primary)]">
        ...
        <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden relative">
          ...
          {/* HEADER */}
          <header className="h-16 shrink-0 border-b border-[var(--border-color)] bg-[var(--bg-app)]/80 backdrop-blur-md sticky top-0 z-20 flex items-center justify-between px-6 transition-all duration-300">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="hidden sm:flex items-center text-sm font-medium">
                ...
                <span className="text-[var(--text-primary)] capitalize">
                  {NAVIGATION_CONFIG.flatMap(g => g.items).find(n => n.id === activeRoute)?.label || activeRoute}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 sm:gap-6">
              ...
              <div className="relative group hidden sm:block">
                ...
                <input 
                  type="text" 
                  placeholder="Search..." 
                  className="h-9 w-60 pl-9 pr-12 rounded-md-custom bg-white border border-[var(--border-color)] text-sm text-[var(--text-primary)] placeholder-[var(--text-secondary)]/60 focus:outline-none focus:ring-2 focus:ring-[var(--primary-gold)]/30 focus:border-[var(--primary-gold)] transition-all shadow-soft-sm"
                />
              ...
              {/* Notifications */}
              <button className="relative p-2 text-[var(--text-secondary)] hover:text-[var(--text-primary)] rounded-full hover:bg-white transition-colors border border-transparent hover:border-[var(--border-color)] shadow-sm">
