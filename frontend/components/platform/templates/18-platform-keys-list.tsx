...
// Auto-converted from 18List Plastform Key.jsx by convert_jsx_to_tsx.py.
...
const ApiKeysPage = () => {
  ...
  // Create / Rotate State
  const [newKeyData, setNewKeyData] = useState({ name: '', scope: 'Full Access' });
  ...
  const handleCreateSubmit = async () => {
    ...
    const newKeyRecord = {
      id: newId,
      name: newKeyData.name || 'Untitled Key',
      keyHash: fullSecret.substring(fullSecret.length - 16), // Store hash locally
  ...
  return (
    <PageContainer maxWidth="default">
      <PageHeader 
        ...
        actions={
          <Button onClick={() => { setGeneratedKey(null); setNewKeyData({name:'', scope:'Full Access'}); setIsCreateOpen(true); }}>
      ...
      <Modal 
        isOpen={isCreateOpen} 
        onClose={() => { if(!generatedKey) setIsCreateOpen(false); }}
        title={generatedKey ? "Save your API Key" : "Create API Key"}
        description={generatedKey ? "Please copy this key now. You won't be able to see it again." : "Generate a new secret key for API access."}
        footer={
          generatedKey ? (
            ...
            <>
              ...
              <Button onClick={handleCreateSubmit} isLoading={isProcessing} disabled={!newKeyData.name}>Generate Key</Button>
            </>
          )
        }
      >
        {!generatedKey ? (
          <div className="space-y-4">
            ...
            <Select 
              ...
              onChange={v => setNewKeyData({...newKeyData, scope: v})} 
            />
          </div>
        ) : (
          <div className="space-y-4">
            <Alert variant="warning" title="Keep this secure">
              Do not share this key in publicly accessible areas such as GitHub, client-side code, etc.
            </Alert>
