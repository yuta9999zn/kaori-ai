...
// Auto-converted from 10Component foundation.jsx by convert_jsx_to_tsx.py.
...
const DataTable = ({  columns, data, loading  }: any) => {
  return (
    <div className="rounded-lg-custom border border-[var(--border-color)] bg-[var(--bg-card)] shadow-soft-sm overflow-hidden">
      <Table>
        <TableHeader>
          <TableRow>
        ...
        <TableBody>
          {loading ? (
            ...
            <TableRow>
              <TableCell colSpan={columns.length} className="h-24 text-center text-[var(--text-secondary)]">No results.</TableCell>
            </TableRow>
          ) : (
            data.map((row, i) => (
              <TableRow key={i}>
...
// --- EXISTING DASHBOARDS (Refactored to use new primitives seamlessly) ---
...
export default function KaoriPlatformShell() {
  const [activeRoute, setActiveRoute] = useState('components'); // Default to components for demo
