...
// Auto-converted from 15Workspace Id Audit.jsx by convert_jsx_to_tsx.py.
...
const MOCK_AUDIT_LOGS = [
  { id: 'evt_901', timestamp: '2026-10-25 14:32:01', actorName: 'Admin User', actorEmail: 'admin@kaori.io', action: 'Created API Key', resource: 'key_live_xyz', status: 'Success', ip: '103.142.12.33', metadata: { keyName: 'Prod Read-Only', scopes: ['read:all'], environment: 'production' } },
  { id: 'evt_902', timestamp: '2026-10-25 10:15:44', actorName: 'Sarah Jenkins', actorEmail: 'sarah@kaori.io', action: 'Deleted Workspace', resource: 'ws_test_09', status: 'Failed', ip: '113.190.55.12', metadata: { reason: 'Insufficient permissions', requiredRole: 'Owner', targetWorkspace: 'ws_test_09' } },
  { id: 'evt_903', timestamp: '2026-10-24 09:22:10', actorName: 'System', actorEmail: 'system@kaori.io', action: 'Billing Updated', resource: 'sub_enterprise', status: 'Success', ip: '127.0.0.1', metadata: { previousPlan: 'Pro', newPlan: 'Enterprise', mrr: 249, billingCycle: 'monthly' } },
  { id: 'evt_904', timestamp: '2026-10-23 16:05:00', actorName: 'Mike Chen', actorEmail: 'mike@kaori.io', action: 'Invited User', resource: 'emily@kaori.io', status: 'Success', ip: '202.168.1.99', metadata: { roleAssigned: 'Viewer', invitationId: 'inv_8821a', expiresAt: '2026-10-30T16:05:00Z' } },
  { id: 'evt_905', timestamp: '2026-10-23 08:12:33', actorName: 'Admin User', actorEmail: 'admin@kaori.io', action: 'Suspended User', resource: 'mike@kaori.io', status: 'Success', ip: '103.142.12.33', metadata: { reason: 'Security review pending', duration: 'indefinite' } },
];
...
const WorkspaceAuditLogPage = ({  setActiveRoute  }: any) => {
  ...
  const filteredLogs = MOCK_AUDIT_LOGS.filter(log => {
    const matchesSearch = log.action.toLowerCase().includes(search.toLowerCase()) || log.resource.toLowerCase().includes(search.toLowerCase());
    const matchesActor = actor === 'all' || log.actorName === actor;
    const matchesEvent = eventType === 'all' || log.action.includes(eventType);
  ...
  const mappedLogs = filteredLogs.map(log => [
    ...
    <div key="actor">
       <div className="text-sm font-medium text-[var(--text-primary)]">{log.actorName}</div>
       <div className="text-xs text-[var(--text-secondary)]">{log.actorEmail}</div>
    </div>,
    <span key="action" className="font-medium text-[var(--text-primary)] whitespace-nowrap">{log.action}</span>,
      ...
      >
        {selectedLog && (
          <div className="space-y-6">
            ...
            {/* Header Info */}
            <div className="flex items-center gap-4 bg-[var(--bg-app)] p-4 rounded-md-custom border border-[var(--border-color)]">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 border ${selectedLog.status === 'Success' ? 'bg-[#F3F9F5] border-[#8FBFA0]/40' : 'bg-[#FDF8F8] border-[#D97C7C]/40'}`}>
                {selectedLog.status === 'Success' ? <CheckCircle2 className="w-5 h-5 text-[#5C856A]" /> : <ShieldAlert className="w-5 h-5 text-[#9B5050]" />}
              </div>
              <div>
                <h3 className="text-base font-semibold text-[var(--text-primary)]">{selectedLog.action}</h3>
                <p className="text-xs text-[var(--text-secondary)]">{selectedLog.timestamp}</p>
              </div>
              <Badge variant={selectedLog.status === 'Success' ? 'operational' : 'error'} className="ml-auto">{selectedLog.status}</Badge>
            </div>
