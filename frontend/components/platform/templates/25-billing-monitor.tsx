...
// Auto-converted from 25Billing Monitor Overview.jsx by convert_jsx_to_tsx.py.
...
const PlatformBillingOverviewPage = ({  setActiveRoute  }: any) => {
  ...
  const tableData = MOCK_TOP_WORKSPACES.map((ws, i) => [
    <span key="name" className="font-medium text-[var(--text-primary)] cursor-pointer hover:underline" onClick={() => setActiveRoute('workspace-details')}>{ws.name}</span>,
           ...
           <Section title="Alerts & Risks">
             <div className="bg-[var(--bg-card)] rounded-md-custom border border-[var(--border-color)] p-4 shadow-soft-sm space-y-3">
                <Alert variant="error" title="Payment Failed">
                  Invoice INV-2026-004 failed for <span className="font-semibold cursor-pointer hover:underline" onClick={() => setActiveRoute('workspace-details')}>Dev Cluster Alpha</span>.
                </Alert>
                <Alert variant="warning" title="Quota Warning">
                  <span className="font-semibold cursor-pointer hover:underline" onClick={() => setActiveRoute('workspace-details')}>Data Analytics Core</span> is at 95% of its API quota.
                </Alert>
