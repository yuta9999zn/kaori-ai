...
const PRESETS: Array<{ labelKey: string; req: SimulateRequest }> = [
  {
    labelKey: 'templates71AuthzSimulate.presetViewerRead',
    req: {
  ...
  {
    labelKey: 'templates71AuthzSimulate.presetOperatorExportPii',
    req: {
  ...
  {
    labelKey: 'templates71AuthzSimulate.presetAnalystMarkActioned',
    req: {
...
export default function AbacSimulatePage() {
  ...
  return (
    <>
      <PageHeader
        title={t('templates71AuthzSimulate.title')}
        description={t('templates71AuthzSimulate.description')}
        actions={
          <>
            <Badge variant="info">{t('templates71AuthzSimulate.phase2Badge')}</Badge>
            <Button variant="tertiary" onClick={() => (window.location.href = '/p2/authz/abac/builder')}>
              <ChevronLeft className="w-4 h-4 mr-1" />
              {t('templates71AuthzSimulate.builderBtn')}
            </Button>
          </>
        }
      />
      ...
      <div className="px-6 lg:px-8 py-6 max-w-[1100px] mx-auto space-y-5">
        ...
        <div className="bg-[var(--bg-card)] rounded-lg-custom border border-[var(--border-color)] p-4 shadow-soft-sm">
          <p className="text-[11px] uppercase tracking-wider text-[var(--text-secondary)] mb-2">{t('templates71AuthzSimulate.presetsLabel')}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormPanel title={t('templates71AuthzSimulate.subjectTitle')} icon={User}>
            <Field label={t('templates71AuthzSimulate.roleLabel')}        value={req.subject.role ?? ''}        onChange={(v) => setSubject('role', v)} placeholder="MANAGER / OPERATOR / ANALYST / VIEWER" />
            <Field label={t('templates71AuthzSimulate.departmentLabel')}  value={req.subject.department ?? ''}  onChange={(v) => setSubject('department', v)} placeholder="sales / marketing / ops / finance" />
            <Field label={t('templates71AuthzSimulate.locationLabel')}    value={req.subject.location ?? ''}    onChange={(v) => setSubject('location', v)} placeholder="HCM / HN / DN" />
            <Field label={t('templates71AuthzSimulate.mfaLabel')}          value={req.subject.mfa_state ?? ''}  onChange={(v) => setSubject('mfa_state', v)} placeholder="enabled / disabled" />
          </FormPanel>

          <FormPanel title={t('templates71AuthzSimulate.resourceTitle')} icon={Database}>
            <Field label={t('templates71AuthzSimulate.tableLabel')}        value={req.resource.table ?? ''}        onChange={(v) => setResource('table', v)} placeholder="gold_revenue / silver_orders..." />
            <Field label={t('templates71AuthzSimulate.sensitivityLabel')}  value={req.resource.sensitivity ?? ''}  onChange={(v) => setResource('sensitivity', v)} placeholder="public / internal / confidential / pii" />
            <Field label={t('templates71AuthzSimulate.ownerTeamLabel')}   value={req.resource.owner_team ?? ''}    onChange={(v) => setResource('owner_team', v)} placeholder="finance / sales / shared" />

            <div>
              <label className="text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider">{t('templates71AuthzSimulate.actionLabel')}</label>
              <select
                value={req.action}
        ...
        <div className="flex justify-center">
          <Button onClick={run} isLoading={running} disabled title={t('templates71AuthzSimulate.runBtnTooltip')}>
            <Activity className="w-4 h-4 mr-2" />
            {t('templates71AuthzSimulate.runBtn')}
        ...
        <div className={cn(
          ...
          {decision ? (
            <>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-[var(--text-secondary)]">{t('templates71AuthzSimulate.pdpDecisionLabel')}</p>
                </div>
              </div>
              <p className="text-sm text-[var(--text-primary)] leading-relaxed">{decision.reason}</p>
              {decision.policy_id && (
                <p className="text-[11px] text-[var(--text-secondary)] mt-2">
                  {t('templates71AuthzSimulate.policyIdLabel')} <span className="font-mono">{decision.policy_id}</span>
                </p>
              )}
              {decision.missing_perms.length > 0 && (
                <div className="mt-3">
                  <p className="text-[11px] uppercase tracking-wider text-[var(--text-secondary)] mb-1">{t('templates71AuthzSimulate.missingPermsLabel')}</p>
              ...
              {decision.matched_rules.length > 0 && (
                <div className="mt-3">
                  <p className="text-[11px] uppercase tracking-wider text-[var(--text-secondary)] mb-1">{t('templates71AuthzSimulate.matchedRulesLabel')}</p>
            ...
            <div className="text-center py-6 text-[var(--text-secondary)]">
              ...
              <p className="text-sm">{t('templates71AuthzSimulate.emptyStateHint')}</p>
            </div>
          )}
        </div>
        ...
        <div className="flex items-start gap-2 p-3 rounded-md-custom bg-[var(--bg-app)]/40 border border-[var(--border-color)] text-xs text-[var(--text-secondary)]">
          ...
          <p>
            {t('templates71AuthzSimulate.footerNote')}
          </p>
        </div>
      </div>
    </>
