...
export default function SchemaSuggestionPage() {
  ...
  async function onRegenerate() {
    ...
    try {
      ...
      setSuccess('Đã tái sinh schema từ Kaori AI.');
    } catch (e: any) {
      setProblem(e);
    } finally {
  ...
  async function onCommit() {
    ...
    try {
      ...
      setSuccess(`Đã tạo ${acceptedTables.length} bảng với ${stats.accepted} cột. Schema có hiệu lực ngay.`);
    } catch (e: any) {
      setProblem(e);
    } finally {
      setSubmitting(false);
  ...
  return (
    <>
      <PageHeader
        title="Đề xuất schema"
        description="Kaori AI quét file/dataset → đề xuất bảng + cột + kiểu dữ liệu."
        actions={
          <>
      ...
      <div className="px-6 lg:px-8 py-6 max-w-[1300px] mx-auto space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-4 items-stretch">
          <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg-custom p-5 shadow-soft-sm">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-serif text-base text-[var(--text-primary)]">Nguồn phân tích</h3>
            </div>
            <div className="flex items-center gap-3 p-3 rounded-md-custom bg-[var(--bg-app)] border border-[var(--border-color)]">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-[var(--text-secondary)] mt-0.5">
                  16.760 dòng · 10 cột · upload 2026-04-30 14:18 · SHA-256 a3f8c…b21d
                </p>
              </div>
          ...
          <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg-custom p-5 shadow-soft-sm flex flex-col justify-center">
            <p className="text-[11px] uppercase tracking-wider text-[var(--text-secondary)] font-medium">Tổng quan đề xuất</p>
            <p className="font-serif text-2xl text-[var(--text-primary)] mt-1">
              {stats.accepted}/{stats.total} cột chọn
            </p>
            <p className="text-xs text-[var(--text-secondary)] mt-1">
              {stats.lowConf > 0 && (
                <span className="text-[var(--state-warning)]">{stats.lowConf} cột confidence &lt; 80%. </span>
              )}
              Review kỹ trước khi commit.
            </p>
          </div>
        </div>
        ...
        <label className="flex items-start gap-3 p-3 rounded-md-custom border border-[var(--border-color)] bg-[var(--bg-card)] cursor-pointer hover:border-[var(--primary-gold)]/40 transition-colors shadow-soft-sm">
          ...
          <div>
            <p className="text-sm font-medium text-[var(--text-primary)]">
              Cho phép gửi mẫu dữ liệu (đã PII-mask) cho AI ngoài để tăng độ chính xác
            </p>
            <p className="text-xs text-[var(--text-secondary)] mt-1 leading-relaxed">
              K-4: workspace cần bật consent. K-5: 100 dòng đầu mask &lt;EMAIL&gt;/&lt;PHONE&gt;/&lt;NAME&gt; trước khi gửi.
              Mặc định Qwen 2.5 nội bộ — không cần consent.
            </p>
          </div>
        </label>
        ...
        <div className="flex items-start gap-2 p-3 rounded-md-custom bg-[var(--bg-app)]/40 border border-[var(--border-color)] text-xs text-[var(--text-secondary)]">
          ...
          <p>
            Schema commit ghi vào <span className="font-mono">decision_audit_log</span> (K-6) với reasoning của từng cột.
            NUMERIC(14,4) cho tiền (K-9) — không bao giờ FLOAT.
          </p>
        </div>
      </div>
    </>
      ...
      <div className="px-5 py-4 border-b border-[var(--border-color)] flex items-center justify-between bg-[var(--bg-app)]">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-[var(--text-secondary)] mt-0.5">
              {t.rows.toLocaleString('vi-VN')} dòng · {acceptedCount}/{t.columns.length} cột chọn
            </p>
          </div>
        </div>
        <Badge variant="info">{t.columns.length} cột đề xuất</Badge>
      </div>
      <div className="overflow-auto">
        <table className="w-full text-sm text-left">
          <thead className="border-b border-[var(--border-color)] text-[11px] font-medium uppercase tracking-wider text-[var(--text-secondary)] bg-[var(--bg-card)]">
            <tr>
              <th className="px-5 py-3 w-10">Chọn</th>
              <th className="px-5 py-3">Tên cột</th>
              <th className="px-5 py-3">Kiểu</th>
              <th className="px-5 py-3">Null?</th>
              <th className="px-5 py-3">Khoá</th>
              <th className="px-5 py-3">Confidence</th>
              <th className="px-5 py-3">Lý do AI</th>
            </tr>
          </thead>
    ...
    <tr className={cn(
      ...
      <td className="px-5 py-3">
        <button
          ...
          aria-label={c.accepted ? 'Bỏ chọn' : 'Chọn'}
        >
