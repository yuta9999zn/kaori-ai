...
export function NotesPanel({ docId }: { docId: string }) {
  ...
  const load = useCallback(() => {
    api<{ items: DocNote[] }>(`/api/v1/document-repository/${docId}/notes`)
      ...
      .catch((e: ProblemDetails) => setError(e?.title || t('dmsNotesPanel.errLoad')));
  }, [docId]);
  ...
  async function add() {
    ...
    try {
      ...
      setDraft('');
      load();
    } catch (e: any) { setError(e?.title || t('dmsNotesPanel.errSave')); }
  ...
  async function remove(noteId: string) {
    try {
      ...
      load();
    } catch (e: any) { setError(e?.title || t('dmsNotesPanel.errDelete')); }
  }
  ...
  return (
    <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg-custom px-4 py-3 space-y-3">
      <h2 className="text-sm font-semibold flex items-center gap-1.5">
        ...
        {t('dmsNotesPanel.heading', { count: notes?.length ?? '…' })}
      </h2>
      ...
      {notes === null ? (
        ...
        <p className="text-xs italic text-[var(--text-secondary)]">{t('dmsNotesPanel.empty')}</p>
      ) : (
          ...
          {notes.map((n) => (
            <div key={n.note_id} className="group border-l-2 border-[var(--primary-gold)]/40 pl-3">
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => remove(n.note_id)} title={t('dmsNotesPanel.deleteTitle')}
      ...
      <div>
        ...
        <textarea ref={taRef} rows={2} value={draft} onChange={(e) => setDraft(e.target.value)}
          placeholder={t('dmsNotesPanel.draftPlaceholder')}
